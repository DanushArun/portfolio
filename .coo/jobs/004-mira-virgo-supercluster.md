...
**Canonical visual bar:** `.coo/jobs/004/refs/quality-bar-storyboard.png` (4-panel storyboard, founder-supplied 2026-05-13). Every visual AC is verified by side-by-side comparison with the matching panel: AC1 against Panel 1 (overview / dormant), AC3 + AC4 against Panel 2 (activation), AC4 return-phase against Panel 3 (plume return), AC5 against Panel 4 (accretion). A render that does not visibly reach this bar is not done.

## Acceptance criteria

- [ ] **AC1 — Structural fidelity.** When `phase === 'W01_MIRA'` and `reveal === 1.0`, the rendered scene shows: (a) a **cosmic web of fine purple/blue filaments** connecting the 5 nodes — not a uniform gas cloud; filaments have visible internal thread structure at viewing distance; (b) exactly 5 knot **cores** distributed along filament intersections, each reading as a **warm white/orange hot core** (not red sphere) contrasted against the cool blue/purple web; (c) atmospheric multi-layer depth (foreground/midground filaments at different densities); (d) a starfield backdrop visible *through* the web, not occluded. No spherical planet, no Aizawa attractor present. The rendered scene must read as a photographic Hubble/Planck-style observational image, not a video-game CGI render. Verified by `visual-verifier` capturing the W01_MIRA mid-panel screenshot at 1440×900 and **comparing side-by-side with `.coo/jobs/004/refs/quality-bar-storyboard.png` Panel 1**. Rejection criteria (any single one fails the AC): filaments look like uniform fog; knot cores read as flat red spheres; palette is monochrome blue (no warm/cool contrast); scene looks like geometric primitives rather than astrophotography.
...
- [ ] **AC4 — Outbound plume.** Within ~0.3s of a knot becoming active, a particle plume (~6k–10k particles) begins emitting from that knot's position. Particles travel outward following a curl-noise-shaped velocity field for ~1.2–1.6s, reaching an apogee at ~3–4× knot radius, then curve back along return paths and reincorporate into the **same** originating knot (locked per Decisions — no cross-knot distribution). Plume is a **sheet-like ribbon** with visible internal texture and wisp structure (per `quality-bar-storyboard.png` Panel 2 and Panel 3), not an isotropic particle spray. Plume contains **both warm (orange, matching knot hue) and cool (cyan) particle bands** flowing together — not single-colour. Return trajectory wraps back to the knot in a curving veil (Panel 3 "data embrace"), not straight-line gravity. Verified visually by `visual-verifier` against Panels 2 and 3 of the storyboard.
...
### 2026-05-13 — Quality bar canonised to 4-panel storyboard

Founder supplied a Gemini-generated 4-panel storyboard fixing the exact visual target. Stored at `.coo/jobs/004/refs/quality-bar-storyboard.png`. All visual ACs (AC1, AC4, AC5) gated against this artifact. Five concrete upgrades over the prior "diffuse blue gas + red spheres" framing:
1. **Cosmic-web filaments** — fine purple/blue thread-mesh connecting nodes, not uniform diffuse gas
2. **Warm/cool palette contrast** — warm white/orange knot cores against cool blue/purple web
3. **Sheet-like ribbon plume** with internal texture — not a particle spray
4. **Mixed warm + cool plume bands** — orange and cyan particles flowing together
5. **Photographic Hubble/Planck multi-layer depth** — not video-game CGI

The reference is the standard. Visual-verifier rejections must cite the specific panel a render fails to match (e.g., "fails Panel 2 — plume reads as circular dots, not sheet ribbon").
