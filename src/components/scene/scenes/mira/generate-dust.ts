...
function haloColor(knot: KnotW, rng: Rng): Vec3 {
  return mixVec(BLUE_HALO, knot.hue, 0.26 + rng() * 0.32);
}

function writeHalo(out: PSet, index: number, knot: KnotW, rng: Rng): void {
  const ptr = index * 3;
  const angle = rng() * Math.PI * 2;
  const radius = (0.34 + Math.abs(gauss(rng)) * 0.92) * knot.scale;
  const stretch = 0.78 + rng() * 0.72;
  ...
  out.densityLevel[index] = 0.16 + rng() * 0.26;
  out.isHalo[index] = 1;
  out.langIndex[index] = LANG_INDEX[knot.lang];
  out.warpParams[ptr] = rng() * 1000;
  out.warpParams[ptr + 1] = 0.006 + radius * 0.004;
